...
export interface AppState {
  ...
  epgGuideCategoryId: string | null;
}
...
export class AppStore {
  ...
  async bootstrap(): Promise<void> {
    ...
    try {
      ...
      if (session) {
        ...
        try {
          ...
          if (credentials) {
            ...
            await contentService.healthCheck().catch(() => {
              // Dashboard/load flows will show a specific error if the provider remains unavailable.
            });
            this.patch({
              credentials,
              screen: "home",
      ...
      if (local) {
        ...
        await contentService.healthCheck().catch(() => {
          // Keep local session available; content calls will surface provider errors.
        });
        this.patch({
          credentials: local,
          screen: "home",
  ...
  openPlayer(session: PlayerSession): void {
    this.patch({ player: session, screen: "player", error: null, epgGuideCategoryId: null });
  }

  openEpgGuide(categoryId?: string): void {
    this.patch({
      screen: "epgGuide",
      epgGuideCategoryId: categoryId ?? null,
      error: null,
    });
  }
  ...
  closePlayer(): void {
    this.patch({
      player: null,
      screen: this.state.detail ? "detail" : this.navToScreen(this.state.navSection),
    });
  }
  ...
  back(): void {
    ...
    if (this.state.screen === "epgGuide") {
      this.patch({ screen: this.navToScreen(this.state.navSection), epgGuideCategoryId: null });
      return;
    }
  ...
  async completeLogin(credentials: XtreamCredentials, accountEmail?: string | null): Promise<void> {
    await contentService.authenticate(credentials);
    await contentService.healthCheck().catch(() => {
      // Credentials are valid enough to authenticate; keep the session and allow retry in UI.
    });
