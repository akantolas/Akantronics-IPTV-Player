...
Αν το CLI δημιουργήσει αρχείο με κενό στο όνομα (`Akantronics IPTV.wgt`), μετονόμασέ το πριν το install:

```powershell
Copy-Item "dist\Akantronics IPTV.wgt" "dist\AkantronicsIPTV.wgt" -Force
tizen install -n AkantronicsIPTV.wgt -s 192.168.1.104:26101 -- dist
```

### 5. Debug

```bash
sdb shell 0 debug akantiptv1.AkantronicsIPTV
```
